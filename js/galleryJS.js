 ...
 */
{
    ...
    class Grid {
        ...
        showContent() {
            ...
            //this.DOM.content.style.backgroundImage = this.DOM.items[this.pos].querySelector('.grid__item-inner').style.backgroundImage.replace(/(\.\.\/images\/gallery)/g, '../images/gallery/large');
            this.DOM.content.style.backgroundImage = this.DOM.items[this.pos].querySelector('.grid__item-inner').style.backgroundImage
                .replace(/url\("(.*)-\d{3}x\d{3}\.jpg"\)/, 'url("$1.jpg")');
    ...
    function loadGallery() {
        fetch('../items/galleryInfo.json')
            .then(response => response.json()) // Parsear la respuesta como JSON
            .then(data => {
                const imageContainer = document.querySelector('.grid'); // Contenedor de la galería
                const images = data.Images;

                // Repetición de las imágenes
                const totalRepetitions = 2; // Duplicados
                let allImages = [];

                // Duplicar el conjunto de imágenes
                for (let i = 0; i < totalRepetitions; i++) {
                    allImages = allImages.concat(images); // Añadir el set de imágenes completo cada vez
                }

                // Mezclar el array de imágenes de forma aleatoria
                const shuffledImages = shuffleArray(allImages);

                // Iterar sobre el array de imágenes aleatorias
                shuffledImages.forEach(imageData => {
                    // Crear un div contenedor de la imagen
                    const div = document.createElement('div');
                    div.classList.add('grid__item');
                    div.setAttribute('data-title', imageData.imageLabel); // Título de la imagen

                    // Crear un div interno para la imagen
                    const innerDiv = document.createElement('div');
                    innerDiv.classList.add('grid__item-inner');
                    innerDiv.style.backgroundImage = `url(${imageData.imageUrl})`; // URL de la imagen

                    // Añadir el innerDiv al div principal
                ...
                // Una vez que las imágenes estén en el DOM, inicializa la galería
